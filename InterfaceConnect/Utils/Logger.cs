...
namespace InterfaceConnect
{
    public class Logger : IDisposable
    {
        ...
        private Logger()
        {
            try
            {
                LogLevel = int.Parse(AppSettingsTool.ReadSetting("LogLevel", "1"));
                CreateLog();
            }
        ...
        {
            string dicPath = AppSettingsTool.ReadSetting("LogPath","");
            if (string.IsNullOrEmpty(dicPath))
            {
                dicPath = System.AppDomain.CurrentDomain.BaseDirectory + "/Log";
            }
            string fileName = string.Format(@"\Connect_{0}.log", DateTime.Now.ToString("yyyyMMddHH"));
        ...
        private void XLogInfo(string message)
        {
            try
            {
                ...
                using (fs = new FileStream(logPath, FileMode.Append, FileAccess.Write, FileShare.ReadWrite))
                {
                    using (sw = new StreamWriter(fs))
                    {
                        sw.WriteLine(String.Format("{2} Info:  DataTime-{0}  Info-{1}", DateTime.Now.ToString(), message, "Web"));
                        sw.Flush();
                    }
                }
            }
        ...
        private void XLogError(string errormsg)
        {
            try
            {
                ...
                using (fs = new FileStream(logPath, FileMode.Append, FileAccess.Write, FileShare.ReadWrite))
                {
                    using (sw = new StreamWriter(fs))
                    {
                        sw.WriteLine(String.Format("{2} Error: DataTime-{0}  Message-{1}", DateTime.Now.ToString(), errormsg, "Web"));
                        sw.Flush();
                    }
                }
            }
